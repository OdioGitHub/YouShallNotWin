...
def check_game_status(board):
    ...
    for t in [1, 2]:
        for j in range(0, 16, 4):
            if [t] * 4 == [board[i] for i in range(j, j+4)]:
                return t
        for j in range(0, 4):
            if board[j] == t and board[j+4] == t and board[j+8] == t and board[j+12] == t:
                return t
        if board[0] == t and board[4] == t and board[8] == t and board[j+12] == t:
            return t
        if board[2] == t and board[4] == t and board[8] == t and board[j+12] == t:
            return t
...
class TicTacToeEnv(gym.Env):
    ...
    def _show_board(self, showfn):
        ...
        for j in range(0, 16, 4):
            ...
            showfn(LEFT_PAD + '|'.join([mark(i) for i in range(j, j+4)]))
            if j < 12:
                showfn(LEFT_PAD + '-------')
